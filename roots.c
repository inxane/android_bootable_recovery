		         ...
		         LOGW("format: %s as %s\n", info->device, info->filesystem);
	                if (strncmp(info->filesystem, "ext4",4) == 0) {					
						///sbin/mke2fs -T ext4 -F -q -m 0 -b 4096 -O ^huge_file,extent /sdcard/cm6/data.img
                           char* args[] = {"/sbin/mke2fs", "-T", info->filesystem, "-F", "-q", "-m", "0", "-b", "4096", "-O", "^huge_file,extent", info->device, NULL};
	                   execv(args[0], args);
	                } else {
                           char* args[] = {"/sbin/mke2fs", "-T", info->filesystem, "-F", "-q", "-m", "0", "-b", "4096", info->device, NULL};
	                   execv(args[0], args);
	                }
